...
const TestResults = () => {
  ...
  const { categoryScores, totalQuestions, answeredQuestions, fullName } = location.state || {};
  ...
  try {
    ...
    if (imgHeight <= pageHeight) {
      ...
      while (remainingHeight > 0) {
        ...
        pageCanvas.height = Math.min(canvas.height - sourceY, (pageHeight / imgHeight) * canvas.height);
        ...
        pdf.addImage(pageImgData, 'PNG', 0, 0, imgWidth, Math.min(pageHeight, remainingHeight));
          ...
          <div className="text-center mb-12">
            ...
            {fullName && (
              <div className="mt-6 flex justify-center">
                <div className="rounded-xl bg-gradient-to-r from-blue-500 to-blue-700 px-8 py-4 flex flex-row items-center shadow-md" style={{lineHeight: '1.5'}}>
                  <span className="text-white text-base font-medium mr-2" style={{lineHeight: '1.5'}}>{t('fullNameLabel')}</span>
                  <span className="text-white text-xl font-bold" style={{lineHeight: '1.5'}}>{fullName}</span>
                </div>
              </div>
            )}
          </div>
